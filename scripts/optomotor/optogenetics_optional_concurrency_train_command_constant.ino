...
// state which the pin will become
// transitorily it's 0 while the decision to turn OFF has been taken but not applied
// transitorily it's 1 while the decision to turn ON has been taken but not applied
int react[N_OUTPUTS] = {0};
int set_array[N_OUTPUTS] = {0};
...
void setup() {
  ...
  //Pulse train to have a pulse of on/off for a limited amount of time
  SCmd.addCommand("R", pulse_train);
  
  //Set to turn on the pin indefinitely
  SCmd.addCommand("S", set);
  
  //Pulse train to have a pulse of on/off for an undefined amount of time (i.e. forever)
  SCmd.addCommand("W", set_pulse_train);
  
  //Unset to turn off the pin
  SCmd.addCommand("U", unset);
  for (unsigned int i = 0; i != N_OUTPUTS; ++i) {
    //Set placeholder value as 2 
    //(0 and 1 are reserved for set and unset scenarios)
    set_array[i]=2;
  }
...
void set_pulse_train_concurrent() {
   char *arg;
   int i = 0;
   int pulse_on = 0;
   int pulse_off = 0;
...
// TODO
void set_pulse_train_not_concurrent() {}

void set_pulse_train() {
  if (CONCURRENT == 1) {
      set_pulse_train_concurrent();
  } else if (CONCURRENT == 0) {
      set_pulse_train_not_concurrent();    
  }
}
...
void train_loop() {
  ...
  for (unsigned int i = 1; i != N_OUTPUTS; ++i){
    ...
    //Check if timers have run to end, if not, subtract tick and keep going
    if(timers[i] != 0){
      timers[i] -= tick;
      if(timers[i] < 50 ){
          timers[i] = 0;
    ...
    //Takes care of counting down the train_on time
    //(has the "on" section of the pulse been on long enough)
    ...
    //If timers have run out or pin is designated to turn off, then turn them off
    if (timers[i] == 0 and set_array[i] != 1) {
        if (DEBUG) {Serial.println("Timer run out");}
...
//    } else if (set_array[i] == 1 and timers[i] == 0) {
      //
    // If the pin should be on according to the previous checks, turn them on
    } else if (react[i] == 1 and set_array[i] != 0) {
      if (DEBUG) {Serial.println("Turn on");}
      digitalWrite(pins[i], HIGH);
      state[i] = 1;
    
    // If the state is on but react is 0, the pin should be turned off
    // The react part is checked by the previous else if statement
    } else if (state[i] == 1) {
